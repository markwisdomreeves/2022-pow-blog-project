...
const createPostCtrl = expressAsyncHandler(async (req, res) => {
  ...
  const isProfane = filter.isProfane(
    req.body.title,
  ...
  if (isProfane) {
    ...
    throw new Error(
      "Creating Failed because it contains profane words and you have been blocked for that"
    );
  }
  ...
  if (
    ...
    throw new Error(
      "Starter account can only create 2 posts. To be able to create more posts, Get more followers."
    );
  }
...
const toggleAddLikeToPostCtrl = expressAsyncHandler(async (req, res) => {
  ...
  //3. check if this user has liked this post or not?
  const isLiked = post?.isLiked;
  //4. check if this user has dislikes this post or not
...
const toggleAddDislikeToPostCtrl = expressAsyncHandler(async (req, res) => {
  ...
  //2. Find the login user
  const loginUserId = req?.user?._id;
  //3. check if this user has already disLikes or not
  const isDisLiked = post?.isDisLiked;
  //4. check if this user already like this post or not
